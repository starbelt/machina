...
**Status:** Draft v0.5 (Layers 1–3 complete, Layer 4 next)
...
**Implemented library functions (as of v0.4):**
...
- **Utilities:** `util.ttp_computation`

**Planned but not yet implemented:**
- **Dynamics:** `dynamics.keplerian`, `dynamics.j2`, `dynamics.cartesian_two_body` — Phase 5
- **Transforms:** `transform.koe_to_cartesian`, `transform.frame_rotation` — Phase 5
...
### Phase 3 — Agent Types and Path Resolution ← NEXT
...
**Phase 3a — Core Transforms and Agent Base:**
- [ ] Implement `QuantityDeclaration` and `ConstraintDeclaration` dataclasses — `machina/agents/declarations.py`
- [ ] Implement `AgentType` base class with declare/build/resolve — `machina/agents/base.py`
- [ ] Implement `transform.koe_to_mee` and `transform.mee_to_koe` factories — `machina/blocks/library/transform.py`
- [ ] Implement `transform.mee_to_eci` factory
- [ ] Implement compiler stub (declare → assign → build → register lifecycle, Python-driven)
- [ ] Tests: element transforms roundtrip, MEE → ECI against known orbits

**Phase 3b — Universal Kepler Solver:**
- [ ] Implement `transform.stumpff_cs` factory (ca.if_else + Taylor expansion near ψ=0)
- [ ] Implement `transform.universal_kepler` factory (ca.rootfinder)
- [ ] Implement `transform.lagrange_coefficients` and `transform.propagate_universal`
- [ ] Tests: verify against classical Kepler (elliptic), hyperbolic case, continuity across e=1, IFT gradient correctness, expand=True compatibility
...
9. **`expand=True` as default:** Should the solver backend default to `expand=True` (converts MX→SX at build time for faster evaluation)? This is a free performance win for problems without callbacks or non-expandable operations, but it's incompatible with `ca.Callback` objects (iteration callbacks). **Lean: default True, disable when callbacks are attached.** Note: `ca.rootfinder` (used for Kepler's equation) compatibility with `expand=True` needs testing.
